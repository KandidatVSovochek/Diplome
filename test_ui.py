import allure
import pytest
from UI.repo_page import RepoPage
from UI.pull_page import Pull_req
from selenium.webdriver.common.by import By
import re
from dotenv import load_dotenv

load_dotenv()


@allure.title("Проверка цвета шрифта")
@allure.story("Цвет шрифта")
@allure.description("Проверка цвета шрифта при введении названия"
                    "репозитория, которое не занято в аккаунте")
@allure.feature("UI")
@allure.severity("NORMAL")
@pytest.mark.UI
def test_text_under_input_field_green(registration):
    with allure.step("Перейти на сайт GitHub и зарегистрироваться"):
        driver = registration
    with allure.step("Перейти на сайт создания нового репозитория"):
        repo_page = RepoPage(driver)
    with allure.step("Ввести названия репозитория"):
        repo_page.new_text("Diplomee")
    with allure.step("Определить цвет надпись под полем ввода"
                     "названия репозаитория"):
        color_text = repo_page.text_green()
        rgb_values = re.findall(r'\d+', color_text)
        r, g, b = map(int, rgb_values[:3])
    with allure.step("Проверка цвета"):
        assert (
                g > r and g > b and g > 120
                ), f"Цвет {color_text} не похож на зеленый"


@allure.title("Проверка цвета шрифта")
@allure.story("Цвет шрифта")
@allure.description("Проверка цвета шрифта при введении названия"
                    "репозитория, которое занято в аккаунте")
@allure.feature("UI")
@allure.severity("NORMAL")
@pytest.mark.UI
def test_text_under_input_field_red(registration):
    with allure.step("Перейти на сайт GitHub и зарегистрироваться"):
        driver = registration
    with allure.step("Перейти на сайт создания нового репозитория"):
        repo_page = RepoPage(driver)
    with allure.step("Ввести названия репозитория"):
        repo_page.new_text("Diplome")
    with allure.step("Определить цвет надпись под полем ввода"
                     "названия репозаитория"):
        color_text = repo_page.text_red()
        rgb_values = re.findall(r'\d+', color_text)
        r, g, b = map(int, rgb_values[:3])
    with allure.step("Проверка цвета"):
        assert (
                r > g and r > b and r > 140
                ), f"Цвет {color_text} не похож на красный"


@allure.title("Проверка текста надписи под полем названия репозитория"
              "при вводе названия на русском языке")
@allure.story("Текст надписи")
@allure.description("Проверка текста надписи под полеп названия репозитория"
                    "при вводе названия на русском языке")
@allure.feature("UI")
@allure.severity("NORMAL")
@pytest.mark.UI
def test_russian_text(registration):
    with allure.step("Перейти на сайт GitHub и зарегистрироваться"):
        driver = registration
    with allure.step("Перейти на сайт создания нового репозитория"):
        repo_page = RepoPage(driver)
    with allure.step("Ввести названия репозитория"):
        repo_page.new_text("Текст")
    with allure.step("Проверка текста"):
        text = driver.find_element(By.CSS_SELECTOR,
                                   "span[class*='ValidationText']").text
        assert "Your new repository will be created as " in text


@allure.title("Проверка цвета значка открытого pull request")
@allure.story("Цвет значка pull request")
@allure.description("Проверка цвета значка открытого pull request")
@allure.feature("UI")
@allure.severity("minor")
@pytest.mark.UI
def test_open_pullrequest(pull_req):
    with allure.step("Перейти на сайт GitHub, зарегистрироваться,"
                     "перейти на страницу репозитория"):
        driver = pull_req
    with allure.step("Проверка цвета элемента"):
        pull_page = Pull_req(driver)
        color_element = pull_page.green_element()
        rgb_values = re.findall(r'\d+', color_element)
        r, g, b = map(int, rgb_values[:3])
        assert (
                g > r and g > b and g > 120
                ), f"Цвет {color_element} не похож на зеленый"


@allure.title("Проверка цвета значка закрытого pull request")
@allure.story("Цвет значка pull request")
@allure.description("Проверка цвета значка закрытого pull request")
@allure.feature("UI")
@allure.severity("minor")
@pytest.mark.UI
def test_close_pullrequest(pull_req):
    with allure.step("Перейти на сайт GitHub, зарегистрироваться,"
                     "перейти на страницу репозитория"):
        driver = pull_req
    with allure.step("Проверка цвета элемента"):
        pull_page = Pull_req(driver)
        color_element = pull_page.red_element()
        rgb_values = re.findall(r'\d+', color_element)
        r, g, b = map(int, rgb_values[:3])
        assert (
                r > g and r > b and r > 120
                ), f"Цвет {color_element} не похож на красный"
